...
const ACTORS = {
  ...
  "youtube-transcript-scraper": {
    apify: "techforce.global~youtube-transcript-scraper",
    label: "YT Transcript",
    kind: "video",
    needs: ["videoUrl"],
...
try {
  if (PAY_TO) {
    ...
    const routes = {
      ...
      "GET /api/youtube-transcript": {
        accepts: {
          ...
          network: CAIP,
          payTo: PAY_TO,
          price: (ctx) => priceFromCtx(ctx, "youtube-transcript-scraper"), // flat per transcript (records capped to 1)
        },
        resource: resourceFor("/api/youtube-transcript"),
        serviceName: "Techforce Agents YT Transcript", // shared prefix "Techforce Agents" => page header; card shows "YT Transcript"
        tags: ["data", "youtube", "transcript", "video", "captions"],
        unpaidResponseBody: () => ({
          contentType: "application/json",
          body: {
            note: "Sample preview. Pay $0.20 per transcript via x402 to receive live results.",
            items: [
              { videoUrl: "https://www.youtube.com/watch?v=dQw4w9WgXcQ", videoId: "dQw4w9WgXcQ", language: "en", isAutoGenerated: false, isTranslated: false, segmentCount: 3, searchResult: [{ start: 0.0, dur: 3.2, text: "We're no strangers to love" }, { start: 3.2, dur: 3.4, text: "You know the rules and so do I" }, { start: 6.6, dur: 3.1, text: "A full commitment's what I'm thinking of" }] },
            ],
          },
        }),
        description: "Full YouTube video transcript on demand — every spoken segment with timestamps. Give a video URL and get clean, structured captions back, in 13 languages with optional auto-translation. Perfect for AI summarization, content repurposing, and research. One transcript per call.\n\nPowered by Techforce Global — explore more at https://techforceglobal.com",
        mimeType: "application/json",
        extensions: {
          ...declareDiscoveryExtension({
            method: "GET",
            ...
            inputSchema: {
              type: "object",
              properties: {
                videoUrl: { type: "string", description: "YouTube video URL (watch, youtu.be, or embed)" },
                language: { type: "string", description: "Preferred transcript language (optional, default en)", enum: ["en", "hi", "gu", "pt", "es", "fr", "de", "it", "ja", "ko", "zh", "ru", "ar"] },
              },
              required: ["videoUrl"],
            },
            output: {
                ...
                properties: {
                  items: {
                    ...
                    items: {
                      type: "object",
                      properties: {
                        videoUrl: { type: "string", description: "Normalized video URL" },
                        videoId: { type: "string", description: "YouTube video ID" },
                        language: { type: "string", description: "Language actually returned" },
                        isAutoGenerated: { type: "boolean", description: "True if YouTube auto-captions" },
                        isTranslated: { type: "boolean", description: "True if auto-translated" },
                        segmentCount: { type: "integer", description: "Number of transcript segments" },
                        searchResult: { type: "array", description: "Segments, each with start, dur, text", items: { type: "object", properties: { start: { type: "number" }, dur: { type: "number" }, text: { type: "string" } } } },
                      },
                    },
                  },
                },
              },
              example: { items: [{ videoUrl: "https://www.youtube.com/watch?v=dQw4w9WgXcQ", videoId: "dQw4w9WgXcQ", language: "en", isAutoGenerated: false, isTranslated: false, segmentCount: 3, searchResult: [{ start: 0.0, dur: 3.2, text: "We're no strangers to love" }] }] },
            },
          }),
        },
      },
    ...
    // YT Transcript — flat per-transcript path (one video/call; records capped to 1 => flat $0.20).
    app.get("/api/youtube-transcript", async (req, res) => {
      const key = "youtube-transcript-scraper";
      const cust = customerToken(req);
      try {
